...
class BedRepository @Inject constructor(
    ...
    fun getBedArchiveList(): Flow<List<Bed>> =
        bedDao.getBedArchiveList().flowOn(Dispatchers.IO).conflate()
    ...
    fun getChangeByBedId(id: String): Flow<List<Changes>> =
        changeDao.getChangeByBed(id).flowOn(Dispatchers.IO).conflate()

    fun getAllChanges(): Flow<List<Changes>> =
        changeDao.getAllChanges().flowOn(Dispatchers.IO).conflate()
    ...
    fun getAllImages(): Flow<List<Gallery>> =
        galleryDao.getAllGallery().flowOn(Dispatchers.IO).conflate()
    ...
    fun getStatisticByBedId(id: String): Flow<List<Statistics>> =
        statisticDao.getStatByBed(id).flowOn(
            Dispatchers.IO
        ).conflate()

    fun getAllStatistics(): Flow<List<Statistics>> =
        statisticDao.getAllStat().flowOn(Dispatchers.IO).conflate()

    suspend fun addNotification(notifications: Notifications) =
        notificationDao.insertNotification(notifications)

    suspend fun updateNotification(notifications: Notifications) =
        notificationDao.updateNotification(notifications)

    suspend fun deleteNotification(notifications: Notifications) =
        notificationDao.deleteNotification(notifications)

    fun getAllNotification(): Flow<List<Notifications>> =
        notificationDao.getAllNotification().flowOn(Dispatchers.IO).conflate()
